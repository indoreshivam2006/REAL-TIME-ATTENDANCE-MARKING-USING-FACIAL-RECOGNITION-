    ...
    async (imageDataUrl: string) => {
      ...
      try {
        ...
        if (data.faces && data.faces.length > 0) {
          ...
          const recognizedFaces = data.faces.filter((f: any) => f.match);
          const unrecognizedCount = data.faces.length - recognizedFaces.length;

          if (recognizedFaces.length > 0) {
            // Add all newly recognized students
            const newNames = recognizedFaces.map((f: any) => f.match.name);
            setRecognizedStudents((prev) => {
              const uniqueNames = [...new Set([...prev, ...newNames])];
              return uniqueNames;
            });

            // Update status with count
            if (recognizedFaces.length === 1) {
              setStatus(`✅ Recognized: ${recognizedFaces[0].match.name}`);
            } else {
              setStatus(`✅ Recognized ${recognizedFaces.length} students`);
            }
          } else {
            setStatus(`❌ ${data.faces.length} face(s) detected but not recognized`);
          }

          // Set all faces data for visualization
